...
// 在每次申请内存时，一次性申请一块稍微大点的内存，具体信息存储在多余的那块内存里
static void* alloc_mem(std::size_t size, const char* file, int line, bool is_array) {
    assert(line >= 0);

    std::size_t s = size + ALIGNED_LIST_ITEM_SIZE;  // 分配字节为size + ALIGNED_LIST_ITEM_SIZE
    ...
    void* usr_ptr = (char*)ptr + ALIGNED_LIST_ITEM_SIZE;    // 实际用到的内存地址
    ...
    // 将ptr插入双向链表，头插法插到new_ptr_list前面
    {
...
// free_pointer函数的大体思路就是在链表中找到要对应节点，删除掉
...
// 每次new时候会把这段内存插入链表，delete时候会把这段内存从链表中移出，
// 如果程序最后链表长度不为0，即为有内存泄漏
int checkLeaks() {
    int leak_cnt = 0;
...
void operator delete(void* ptr) noexcept { free_pointer(ptr, _DEBUG_NEW_CALLER_ADDRESS, false); }   // 重载delete函数
