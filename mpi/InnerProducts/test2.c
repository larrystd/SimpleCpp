...
void serial_product_in_block(float** block, int rows, int m, float** results)
{
	int i, j;
    /*
...
#define pp(process) ((process) > my_id ? (process) - 1 : (process))
	...
	if (is_master){
		...
		{
			final_results = create_triangle_matrix(n , n );	// 创建一个三角矩阵
	...
	if (has_parallism)
	{
		...
		while (1)
		{
			...
			for(i = 0; i < rows_per_block; i++)
			{
				...
				{
					processor_results[i][j - i + rows_per_block * current_iter] = product(block_left[i], block_right[j], m);
				}
			}
		...
		if (is_master)
		{
			...
			while (1)
			{
				...
				for (i = 0; i < rows_per_block; i++)
				{
					...
					{
						final_results[j - normal_count][n - j] = processor_results[i][j];
					}
	...
	if (is_master)
	{
		if (has_parallism)
		{
			...
			print_results(final_results, n, n );
		...
		final_results_serial = create_triangle_matrix(n , n);
		serial_product_in_block(full_matrix, n, m, final_results_serial);
		printf("Done.\n");

		printf("Results from serial computation:\n");
		// Print out results from serial computation
		print_results(final_results_serial, n, n);
