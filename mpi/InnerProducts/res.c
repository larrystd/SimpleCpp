...
{
	int my_rank, num_procs, prev_rank, next_rank;
	int *indices = NULL;
	...
	int proc_result_nbs = 0;
	float** proc_result = NULL;
...
#define pp(process) ((process) > my_rank ? (process) - 1 : (process))
	...
	if (is_master){
		...
		for (i = 0; i < n; i++)
		{
			for (j = 0; j < m; j++)
			{
				//full_matrix[i][j] = (float)((double)rand());		// 随机生成矩阵
		...
		{
			final_result = create_triangle_matrix(n , n );	// 创建一个三角矩阵
	...
	if (has_parallism)
	{
		...
		while (1)
		{
			...
			if (current_iter < iter_count)
			{
				...
				MPI_Isend(block_right[0], rows_per_block * m, MPI_FLOAT, prev_rank, 0, MPI_COMM_WORLD, &requests[pp(prev_rank)]);
				MPI_Recv(block_right[0], rows_per_block * m, MPI_FLOAT, next_rank, MPI_ANY_TAG, MPI_COMM_WORLD, &statuses[pp(next_rank)]);
			}
			else
			{
				break;
		...
		if (is_master)
		{
			...
			while (1)
			{
				...
				{
					MPI_Recv(proc_result[0], proc_result_nbs, MPI_FLOAT, current_iter, MPI_ANY_TAG, MPI_COMM_WORLD, &statuses[pp(i)]);  // 接收消息
				}
				else
				{
					break;
