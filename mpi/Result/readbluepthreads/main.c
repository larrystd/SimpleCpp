 ...
 */
int rows_for_process(int process_count, int row_count, int tile_row, int current_id) {
...
void usage(char const *const message, ...) {
    ...
    fprintf(stderr, "red_blue [thread num] [board size] [tile size] [threshold (float 0~1)] [maximum iterations]\n");
    fprintf(stderr, "Required: [board size] %% [tile size] = 0; 1 <= [thread num] <= [board size] / [tile size]\n");
    fprintf(stderr, "\n");
}
...
void *process_thread(void *arg) {
    ...
    do {
        ...
        barrier_wait(&sync->barrier);   // wait all thread finish move_red，barrier
        ...
        barrier_wait(&sync->barrier);   // wait all thread finish
...
int main(int argc, char *argv[]) {
    ...
    if (params.p > 1) {
        ...
        for (int i = 0; i < params.p; i++) {
            ...
            ctx->id = i;
            ctx->start = rp;
            rp += rows_for_process(params.p, params.n, params.t, i);
            ctx->end = rp - 1;
        }
